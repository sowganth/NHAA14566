...
from app.schemas.output import DataQuality, FeatureContribution
...
class SVIEngine:
    ...
    def evaluate(
        ...
        # Build map of all evaluable factors and their raw values
        ...
        # Identify present vs missing features (excluding suicidal/self-harm/immediate_danger which are safety overrides)
        ...
        for k in expected_features:
            ...
            if val is None:
                ...
                present_factors_val = float(val)
                present_features[k] = present_factors_val
        ...
        # Calculate sum of weights for present features for re-normalization
        ...
        if sum_present_weights > 0:
            for k in expected_features:
                ...
                raw_score = eval_factors.get(k)
                if k in present_features:
                    # Dynamically re-normalize weight so sum of weights of available features equals 1.0
    ...
    def _calculate_confidence(
        ...
        """
        Calculate assessment confidence score between 0.0 and 1.0 based on data quality,
        completeness, and critical indicator presence.
        """
        # Baseline confidence from completeness (60% weight)
        conf = 0.60 * completeness

        # Presence of high-impact indicators (fear, threat, trauma, stress)
        ...
        # Consistency check: related emotions shouldn't have wild unexplained contradiction
